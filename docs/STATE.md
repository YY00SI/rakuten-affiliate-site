...
## 次のアクション
1. 公開後にトップ、スマートロック、ポータブル電源、アクションカメラ、高級ヘアドライヤー、高級炊飯器、sitemapを確認する。
2. GSCでsitemapを再送信し、上記4記事をURL検査する。インデックス申請は未登録URLだけに限定する。
3. GA4イベント一覧で `rakuten_affiliate_click` の存在とページ別値を確認し、未発火なら計測修正を独立Actとして実施する。
4. 2026-08-07・08-14・08-31にGSC表示、検索クリック、GA4自然検索、楽天クリックを同一定義で取得する。
5. 2026-08-14に関連記事実験、08-31に31記事実験を `scale` / `continue` / `redesign` / `stop` で判定する。

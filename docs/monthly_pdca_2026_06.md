...
- 楽天の注文明細は `対象データが見つかりませんでした` のため `成果商品: なし`。また、楽天管理画面の現行UIでは `商品別クリック` 導線を確認できず、今回は未取得のまま据え置いたため、個別商品の入れ替え判断は保留のまま。
...
- GSC の `未登録 15` は `2026-06-12` 時点の古い集計だった。可視10URLを個別URL検査した結果、`audio-glasses` は登録済み、残る9URLは未登録を確認し優先クロールキューへ追加したため、次回は集計値より URL 検査結果の変化を優先して確認する。
- GA4 は公開記事から楽天リンクを実クリックしても `rakuten_affiliate_click` が確認できず、公開ページでも `window.gtag` / `window.dataLayer` が不在だった。テンプレートを修正し `window.gtag` 明示束縛と `sendBeacon` フォールバックを追加したため、次回は公開反映後の再計測が必要。
...
2. 商品別クリックに基づく商品入れ替え
理由: 楽天管理画面の現行UIで `商品別クリック` 導線を確認できず、商品単位の判断根拠が不足しているため。
3. GA4 管理画面での `rakuten_affiliate_click` キーイベント化
理由: 今回はサイト側の発火ロジック補強とフォールバック追加までに留め、公開反映後の再計測確認を先に行うため。
...
1. 公開反映後に GA4 管理画面で `rakuten_affiliate_click` を再確認し、必要ならキーイベント化する。
2. GSC で優先クロールキューへ追加した 9 URL の反映結果を確認する。
3. `商品別クリック` は現行UIで取得不能の可能性が高いため、当面は注文詳細・ショップ別・GA4・GSC の組み合わせで代替判断する。
4. 2026-07中旬時点で `portable-gaming-pc` 周辺のクリック推移を見て、型番別・悩み別記事追加の可否を判断する。

## ブラウザ追補 2026-06-29

- GSC URL 検査で確認した可視10URL
  - 登録済み: `audio-glasses`
  - 未登録かつ優先クロールキュー追加済み: `action-camera`, `facial-device`, `auto-cooker`, `ai-drone`, `massage-gun`, `electric-shaver`, `home/`, `beauty/`, `work/`
- 楽天アフィリエイト管理画面はログイン済みで今月クリック5・成果0を再確認したが、`商品別クリック` は現UIで導線を特定できなかった。
- 公開記事 `portable-gaming-pc` から楽天リンクの実クリックを行っても GA4 側で `rakuten_affiliate_click` は見えなかったため、`templates/base.html` に `window.gtag` 明示束縛と `sendBeacon` フォールバックを追加した。
- 追補後の品質ゲートは `validate_articles.py errors=0/warnings=0`、`build_site.py` 全76記事ビルド完了、`audit_site.py errors=0/warnings=0`。

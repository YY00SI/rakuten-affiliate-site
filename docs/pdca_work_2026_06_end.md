...
### 実施内容
- Google Search Console にログイン済みであることを確認し、対象プロパティ `https://yy00si.github.io/rakuten-affiliate-site/` を開いた。
- `ページのインデックス登録` レポートは `最終更新日: 2026/06/12` で止まっていたため、可視10URLを URL 検査で個別確認した。
- `audio-glasses` は登録済みを確認した。
- `action-camera` / `facial-device` / `auto-cooker` / `ai-drone` / `massage-gun` / `electric-shaver` / `home/` / `beauty/` / `work/` は未登録を確認し、URL 検査から `インデックス登録をリクエスト済み` まで進めた。
- GA4 にログイン済みであることを確認し、公開記事 `portable-gaming-pc` から楽天リンクを実クリックしたうえでリアルタイムとイベント管理を確認した。
- 楽天アフィリエイト管理画面にログイン済みであることを確認し、今月値 `クリック5 / 成果0 / 売上0円 / 報酬0円` を再確認した。

### ブラウザ確認結果
- GSC の `未登録 15` は古い集計で、少なくとも可視10URLのうち1URLはすでに登録済みだった。
- 未登録だった可視9URLは優先クロールキューへ追加済み。
- GA4 は公開記事から楽天リンクを実クリックしても `rakuten_affiliate_click` を確認できなかった。
- 公開ページには `gtag.js` 読込タグと初期化スクリプトが出力されていたが、ブラウザ実行状態では `window.gtag` / `window.dataLayer` が不在だった。
- 楽天管理画面の現行UIでは `商品別クリック` 導線を特定できず、今回も未取得のまま据え置いた。

### 追加実装
- `templates/base.html` の GA4 初期化を `window.gtag` 明示束縛へ変更した。
- `rakuten_affiliate_click` は `window.gtag` 不在時でも `navigator.sendBeacon` で `https://www.google-analytics.com/g/collect` へ送るフォールバックを追加した。

### 追加品質ゲート
- `validate_articles.py`: `errors=0`, `warnings=0`
- `build_site.py`: 全76記事ビルド完了
- `audit_site.py`: `errors=0`, `warnings=0`
- 補足: stale ディレクトリ削除警告は継続するが、今回も生成結果と監査結果には影響しなかった。

### 判断
- インデックス問題は「未登録 15 をそのまま信じる」段階ではなく、「URL 検査の実態と再クロール依頼の反映待ち」段階へ進んだ。
- `商品別クリック` は現行UI制約で再取得不能のため、次回PDCAでは未取得扱いを維持しつつ、注文詳細・ショップ別・GSC・GA4の代替データで判断する。
- GA4 は UI 設定不足ではなく、公開側の初期化不整合が主因と判断し、先にテンプレート修正を優先した。
